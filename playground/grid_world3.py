...
def epsilon_greedy(e, s, V, W):
    if np.random.rand() < e:
        ...
        #Q, _ = forward(np.hstack((np.tile(s, (3,1)), np.asarray(env._actions).reshape((-1, 1)))),V,W)
        all_actions = env._actions
        #Q, _ = forward(np.hstack((np.tile(s, (4,1)), np.arange(len(env._actions)).reshape(4,1))),V,W)
    ...
    for ep in range(0, num_episodes):
        ...
        for traj in episode:
            ...
            for i in range(step_count, n_step):
                reward = episode[i][2]
                G_t += reward*(gamma**(i-step_count))
            #G_t = max(min(G_t, 20), -20)
            G_t_lst.append(G_t)
            #gamma_n = n_step-step_count
            #network_update(state,action,G_t,next_state,next_action,(gamma**gamma_n),V,W,rh,ro)
            step_count += 1
...
def sarsa_approx_run():
    ...
    #value_function = td_pred(num_episodes=100, gamma=0.99)
    #plot_value_function(value_function, title="Value function")
    V, W , all_rewards = sarsa_controlNFA(num_episodes=500, gamma=0.9, n=1, lr=0.001, batch_size=1000)
    print_policy(V,W)
